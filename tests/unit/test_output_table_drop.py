...
class TestTableDropsUnshowableColumns:
    ...
    def test_the_readable_floor_is_eight_cells(self, capsys, monkeypatch):
        # Pinned from both sides on the `taps` shape, where NAME (39, kept)
        # and ITEMS (5, numeric) are fixed and UPDATED gets what is left.
        # 56 leaves it 8 cells and it stays; 55 leaves it 7 and it goes,
        # because 7 cells of "2 hours ago" is mostly ellipsis.
        monkeypatch.setenv("COLUMNS", "56")
        output.table(TAP_ROWS, headers=TAP_HEADERS, keep=("NAME",))
        wide = capsys.readouterr().out
        assert "2 hours…" in wide and max(widths(wide)) == 56
        monkeypatch.setenv("COLUMNS", "55")
        output.table(TAP_ROWS, headers=TAP_HEADERS, keep=("NAME",))
        narrow = capsys.readouterr().out
        assert "2 hour" not in narrow and max(widths(narrow)) == 46
    ...
    def test_drop_order_is_right_to_left_not_widest_first(
        ...
        # fits even with 0 and 1 squeezed to the floor, so one column has to
        ...
        assert "aaaaaaaaaa" in printed and "bbbbbbbbbb" in printed
        assert max(widths(printed)) <= 24
...
class TestFitColumns:
    ...
    def test_drops_the_rightmost_droppable_column(self):
        show, fitted = output._fit_columns([20, 20, 5], [False] * 3, avail=24)
        assert show == [0, 1]
        assert sum(fitted) + 2 * (len(fitted) - 1) <= 24

    def test_a_protected_column_is_stepped_over(self):
        show, _ = output._fit_columns([20, 20, 5], [False] * 3, avail=24,
                                      protected=[2])
        assert show == [0, 2]
    ...
    def test_a_surviving_column_is_never_below_the_readable_floor(self):
        # The defect in one line. Shrinking alone reaches 24 by squeezing both
        # wide columns under the floor — widths at which the cell is mostly
        # ellipsis — so the third column goes instead and the survivors stay
        # readable.
        squeezed = output._fit_widths([20, 20, 5], [False] * 3, 24)
        assert min(squeezed[:2]) < output._MIN_COL
        _, fitted = output._fit_columns([20, 20, 5], [False] * 3, avail=24)
    ...
    def test_separator_width_is_honoured(self):
        ...
        # more cell per gutter and drop one step sooner.
        assert output._fit_columns([20, 20, 5], [False] * 3, avail=26,
                                   sep=2)[0] == [0, 1, 2]
        assert output._fit_columns([20, 20, 5], [False] * 3, avail=26,
                                   sep=3)[0] == [0, 1]
