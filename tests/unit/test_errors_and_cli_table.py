...
EXPECTED_GROUP_SIZES = {"pkg": 13, "find": 9, "info": 10, "tap": 4,
                        "ai": 8, "chk": 15, "cfg": 12, "team": 6}


class TestCommandTable:
    def test_exactly_77_commands(self):
        assert len(cli.COMMANDS) == 77
...
class TestMainDispatch:
    ...
    def test_no_args_prints_help(self, sandbox, capsys):
        assert cli.main([]) == 0
        out = capsys.readouterr().out
        assert "77 commands · 8 groups" in out
    ...
    def test_help_word(self, sandbox, capsys):
        assert cli.main(["help"]) == 0
        assert "77 commands · 8 groups" in capsys.readouterr().out
