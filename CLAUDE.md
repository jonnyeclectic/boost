...
- **Dense reuse is decided at TWO levels, and the second one is why `chunks`
  ...
  112,081 chunks for 659 chunks of actual change (**170x**), and **10 of the 19
  changed nothing boost indexes at all** — badge JSON, star-history SVGs, CI
  YAML — for 40.0% of the bill. Three rules make it safe, and each was a real
  bug waiting: an entry is reused only when **every** stored chunk carries the
  current digest (a half-written entry must not merge two versions' vectors);
  `_prune_stale_entries` drops rows for entries the catalog dropped (whole-tap
  deletion used to sweep those for free); and a **missing digest is never a
  match**, because two absences are two unknowns. The digest travels in a shard
  too — without it an imported shard re-embeds on the next build, spending the
  CPU the download existed to save.
