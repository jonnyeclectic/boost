...
def discover_subagents(skill_dir: Path, entry_path: Path | None = None) -> list[AgentSpec]:
    """Discover a skill's subagents for multi-agent adaptation.

    A subagent is a Markdown file (other than ``SKILL.md`` or ``entry_path``,
    the file the caller is itself adapting) that lives under an ``agents/`` or
    ``subagents/`` directory *strictly beneath* ``skill_dir`` and carries both a
    frontmatter ``name`` and ``description``. Files are visited in sorted path
    order so the emitted crew/graph is deterministic. Returns the subagents as
    :class:`AgentSpec`s — an empty list for a flat single-agent skill or a
    missing directory.

    Callers must only pass a SKILL.md-rooted ``skill_dir`` (the skill's own
    directory) — passing the shared directory of a flat, non-SKILL.md workflow
    item (e.g. a registry-wide ``agents/`` dir) would otherwise fabricate a
    crew out of every sibling file in that directory.
    """
    specs: list[AgentSpec] = []
    if not skill_dir or not skill_dir.is_dir():
        return specs
    entry_resolved = entry_path.resolve() if entry_path is not None else None
    for path in sorted(skill_dir.rglob("*.md")):
        if path.name == "SKILL.md":
            continue
        if entry_resolved is not None and path.resolve() == entry_resolved:
            continue
        rel_parts = path.relative_to(skill_dir).parts[:-1]
        if not (SUBAGENT_DIRS & {part.lower() for part in rel_parts}):
            continue
        try:
