...
def render_runner(name: str, description: str, body: str,
    ...
    # render_agents_sdk's own banner names `boost adapt NAME --to agents-sdk`,
    # which cannot regenerate this file — it produces a short Agent module,
    # not the runner (with its tool defs and Runner.run_sync call) built
    # below. Restamp the inherited first line with the command that actually
    # regenerates what this function returns.
    #
    # A literal newline in `name`/`tgt` would otherwise end the `#` comment
    # mid-banner and hand the rest of the line to the parser as code — the
    # same class of bug this function exists to avoid elsewhere (`_py_str`
    # for the prompt below). A comment tolerates backticks/quotes/`%`
    # unescaped; only line breaks need neutralizing here.
    safe_name = name.replace("\n", " ").replace("\r", " ")
    safe_tgt = tgt.replace("\n", " ").replace("\r", " ")
    banner = ("# Generated by `boost run %s%s --print`. Do not edit by hand.\n"
              % (safe_name, " " + safe_tgt if safe_tgt else ""))
    _, _, rest = brain.partition("\n")
    brain = banner + rest
