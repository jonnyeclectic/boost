...
def cmd_verify(argv):
    ...
    for kind, name, entry in _iter_installed_all(
        ...
        passed = integrity.verification_passed(status, missing_fields, commit_pin)
        results.append({"name": name, "kind": kind, "status": status,
                        "scope": "user", "missing_fields": missing_fields,
                        "commit_pin": commit_pin, "passed": passed})
    ...
    for r in results:
        ...
        if not r["passed"] and role == "success":
            # status alone reads "ok", but missing lock fields or a drifted
            # commit pin still counts this row among the failures below — the
            # green token must not contradict that.
            role = "danger"
