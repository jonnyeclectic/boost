...
<b>Measured after #865 shipped, and it does not show the defect.</b> The headless run above, 12 router-trivial prompts per arm (the 3 fixtures plus 9 trivial prompts from one real history), counted <code>bmad-*</code> spawns by the main agent: <b>0/12 with no personas, 0/12 with the shipped <code>Use PROACTIVELY</code> descriptions, 0/12 with the fixed ones</b>. At 0/12 the 95% Wilson upper bound is 24%, so this rules out a common spawn, not a rare one. The shipped arm spawned no more than the no-persona arm, which is this card's own stop condition: the fix is harmless, and there was no delegation for it to remove. Two limits. Each run stopped after 4 main-agent tool calls, and 7 of the 12 shipped-arm runs hit that cap, so a spawn later in a long session would not be counted. And the fourth arm, routed prompts with the real hook, measured nothing: in the nested sandboxed session the hooks failed with <code>EPERM</code>, all 6 runs ended in about a second, and no banner reached the context. So whether the fixed wording still delegates when a banner names the persona is <b>not established</b>; re-run that arm outside a sandbox. Cost: $6.20 over the runs that reported one, a floor, since runs stopped early report none.
