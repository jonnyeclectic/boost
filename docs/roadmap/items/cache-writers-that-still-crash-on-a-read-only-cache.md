...
<b>After review.</b> Three writers of the same class. <code>untap</code> in a read-only cache dir warns and names <code>boost clean</code> instead of exiting 70 on the tap's cache file. <code>rebuild_tap</code> tolerates a cache dir it cannot create under a read-only <code>~/.boost</code>, so <code>search</code> and <code>info</code> go from 70 to 0 and <code>reindex</code> reaches its own error naming <code>~/.boost</code>. TAB completion rebuilds an unreadable <code>_names.txt</code> instead of offering nothing. <code>update</code> in that last state still exits 70, in <code>journal.log</code>'s own <code>ensure_dirs</code>.
