...
<b>After review.</b> Three writers of the same class. <code>untap</code> in a read-only cache dir warns and names <code>boost clean</code> instead of exiting 70 on the tap's cache file. <code>rebuild_tap</code> tolerates a cache dir it cannot create under a read-only <code>~/.boost</code>, so <code>search</code> and <code>info</code> go from 70 to 0 and <code>reindex</code> reaches its own error naming <code>~/.boost</code>. TAB completion rebuilds an unreadable <code>_names.txt</code> instead of offering nothing. Still open in the state of a read-only <code>~/.boost</code> with no cache dir, and the same on <code>main</code> before this card: <code>update</code>, <code>heal</code> and <code>doctor</code> exit 70 with crash reports (<code>update</code> in <code>journal.log</code>'s own <code>ensure_dirs</code>). There, <code>heal --dry-run</code> exits 0 while <code>heal</code> exits 70, because main's cache-dir check needs the dir to exist. That state is carded as <code>read-only-boost-home-with-no-cache-dir</code>.
