...
impact: Med
wow: 3
note: "fixed by #889 (the cache is replaced, not rewritten in place: search/browse/info/update/heal 70 → 0 for mode 400 and 000) and the #909 train (b1b1bca, eeb2c76: doctor and heal both name an unwritable cache dir; heal no longer claims a rebuild it did not do) — re-measured on a316c6b"
order: 239
...
<b>Shipped.</b> Re-measured on <code>a316c6b</code> against <code>16de514</code> (the parent of #889's fix), one fixture tap, every command run unpiped against the same bad cache restored before each run. On the old code, mode 400 with a stale <code>format</code> and mode 000 either way gave <code>search</code>/<code>browse</code>/<code>info</code>/<code>update</code>/<code>heal</code> = 70/70/70/70/70 (<code>PermissionError</code>, crash report) while <code>doctor</code> said "● healthy", exit 0. Mode 400 with a current format already crashed <code>update</code> and <code>heal</code> at 70. On main all of those exit 0 in all four cases, and no run printed a traceback. #889 is the fix: <code>rebuild_tap</code> writes through <code>util.atomic_write_text</code>, and a rename needs only the <i>directory</i> writable, so the first command that rescans the tap replaces the 400 or 000 file with a fresh 600 one (a 000 file forces that rescan on any load; a current-format 400 file is served as is by <code>search</code>, <code>browse</code> and <code>info</code> until <code>update</code> or <code>heal</code> rescans it). <code>tests/unit/test_catalog.py::TestACacheBoostCannotWrite</code> pins both modes; the 000 case is <code>test_an_unreadable_cache_file_is_replaced_not_refused</code>, which fails if <code>_cached_tap</code> stops treating a failed read as a miss. <code>boost update &lt;tap&gt;</code> is a working remedy again, and so are <code>search</code> and <code>heal</code>. <code>doctor</code> still says "cloned &amp; cached" over a 000 file, and it is right in effect: the next load repairs the file silently at exit 0, so there is no longer a failure for it to report. Flagging the file mode, as an unmerged draft on this card did, would name a state that fixes itself. The one shape a rename cannot fix is a read-only cache directory holding the bad file. There <code>search</code>, <code>browse</code>, <code>info</code> and <code>update</code> warn once per tap and serve a fresh scan at exit 0 (<code>reindex</code> is not among them: see below). <code>doctor</code> exits 1 naming the directory (#889). <code>heal</code> and <code>heal --dry-run</code> exit 1 with <code>chmod u+w ~/.boost/cache</code> (b1b1bca, eeb2c76 in #909). After that chmod, the next load replaces the file. So doctor and heal no longer contradict each other on any shape of the <i>tap cache file</i>. Two writers into the same directory still crash on a file or directory boost cannot write, and are carded separately as <code>cache-writers-that-still-crash-on-a-read-only-cache</code>: <code>complete.refresh_names</code> writes <code>_names.txt</code> in place, and <code>reindex</code> creates its postings temp file in a read-only cache directory.
