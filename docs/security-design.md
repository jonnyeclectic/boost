...
## VEX: accounting for findings that don't affect boost

Every tool in the table above finds things it does not fix — a CodeQL query
firing on a pattern that isn't actually sensitive here, a gitleaks match on a
string that was never a real credential, a zizmor rule on a trigger that
carries its own mitigation. Each dismissal already has a written reason living
next to the code it describes: a `# codeql[rule-id]` suppression comment, a
`.gitleaks.toml` allowlist entry, an ignore in `.github/zizmor.yml`. None of
that was a document a consumer could ingest, which is exactly what OSPS
Baseline's `OSPS-VM-04.02` asks for: *"While active, any vulnerabilities in
the software components not affecting the project MUST be accounted for in a
VEX document, augmenting the vulnerability report with non-exploitability
details."*

[`docs/vex/openvex.json`](vex/openvex.json) is that document — an
[OpenVEX](https://openvex.dev/) 0.2.0 feed, published at
<https://jonnyeclectic.github.io/boost/vex/openvex.json>. It is generated by
`scripts/build_vex.py` from one Markdown file per statement under
[`docs/vex/statements/`](vex/statements/) — the same one-file-per-item
convention `docs/roadmap/items/*.md` uses, so two people adding two different
findings edit two different files rather than one shared JSON blob.
`make generate` regenerates it; `make lint` and CI both run
`build_vex.py --check` and fail if the committed document has drifted from
its source files.

Every statement in it today is sourced from a real, already-dismissed finding
in this repository, not an invented one — the print-side CodeQL
`py/clear-text-logging-sensitive-data` finding on `trust add`'s fingerprint
line, the gitleaks allowlist entry for `tests/unit/test_secretscan.py`'s
synthetic fixtures, and the two categories of `zizmor` ignores in
`.github/zizmor.yml`. **boost has never had a CVE**, so the document names no
CVE or GHSA identifier — each statement's `vulnerability.name` is the
scanner's own rule id (`codeql:...`, `gitleaks:...`, `zizmor:...`) instead. An
empty-but-honest VEX document would have been the right call had none of these
findings existed to describe; naming a vulnerability that was never reported
would not have been.
